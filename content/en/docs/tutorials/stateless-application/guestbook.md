...
If there are not any replicas running, this Deployment would start the two replicas on your container cluster. Conversely, if there more than two replicas are running, it would scale down until two replicas are running. 
